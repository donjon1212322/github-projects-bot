            ...
            try:
                ...
                if data.get("total_count", 0) > 0:
                    ...
                    if repo.language:
                        ...
                        graphql_data = await fetch_github_graphql(session, graphql_query)
                        contributors_count = (
                            graphql_data.get("data", {}).get("repository", {}).get("collaborators", {}).get("totalCount", 0)
                            if graphql_data and graphql_data.get("data", {}).get("repository", {}).get("collaborators")
                            else 0
                        )
                        ...
                        open_issues_count = (
                            graphql_data.get("data", {}).get("repository", {}).get("openIssues", {}).get("totalCount", 0)
                        )
                        ...
                        projects.append({
                            ...
                            "readme_summary": summary_data["summary"],
                            "key_features": summary_data["key_features"],
                            "primary_use_case": summary_data["primary_use_case"],
                            "open_issues": open_issues_count,
                            "cover_image_prompt": summary_data["cover_image_prompt"]
                        })
